...
def Evaluate(eq, solve = False, replace = False, guess = 10):
  ...
  if solve:
    return NewtonMethod(eq, guess, 5, True)
  ...
  while set(eq) & frozenset("/*+-") and not isFloat(eq):
    ...
    print(f"eq, {eq[progress:]}")
    
    i = 0
    while not opIsFloat(eq[:progress][i:]): i += 1
    first = eq[:progress][i:]
...
def opIsFloat(string):
  return isFloat(string) and ((complex(toFloat(string)).real == 0 and 
                               complex(toFloat(string)).imag != 0) or 
                              (complex(toFloat(string)).real != 0 and 
