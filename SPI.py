...
spi.configure(baudrate=200_000_000)
...
def MYsend_render(render, k = 0):
    ...
    for y in range(0, len(render.bitmap), 8):
        ...
        for x in range(render.width):
            send_data((0b10000000 if (render.bitmap[y + 7] >> (render.width - x - 1) & 1) else 0) |
                      (0b01000000 if (render.bitmap[y + 6] >> (render.width - x - 1) & 1) else 0) |
                      (0b00100000 if (render.bitmap[y + 5] >> (render.width - x - 1) & 1) else 0) |
                      (0b00010000 if (render.bitmap[y + 4] >> (render.width - x - 1) & 1) else 0) |
                      (0b00001000 if (render.bitmap[y + 3] >> (render.width - x - 1) & 1) else 0) |
                      (0b00000100 if (render.bitmap[y + 2] >> (render.width - x - 1) & 1) else 0) |
                      (0b00000010 if (render.bitmap[y + 1] >> (render.width - x - 1) & 1) else 0) |
                      (0b00000001 if (render.bitmap[y + 0] >> (render.width - x - 1) & 1) else 0))
...
def send_render(render, k=0):
    ...
    pad_rows = k % 8
    if pad_rows:
        render.bitmap = [0] * pad_rows + render.bitmap
    k -= pad_rows  # Ensure k is a multiple of 8
    ...
    for y in range(0, height, 8):
        ...
        for x in range(width):
            ...
            for b in range(8):
                row = y + b
                if row < height and (bitmap[row] >> width - x - 1) & 1:
                    byte |= 1 << b
            send_data(byte)